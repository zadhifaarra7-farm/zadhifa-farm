...
export default function InvoicePage({ params }: { params: { id: string } }) {
    ...
    if (loading) return <div className="min-h-screen bg-[#0a0f0d] p-8 text-white">Memuat Invoice...</div>;
    if (!order) return <div className="min-h-screen bg-[#0a0f0d] p-8 text-white">Invoice Tidak Ditemukan</div>;
            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div className="bg-white text-black p-12 rounded-xl shadow-2xl print:shadow-none print:rounded-none">
                    ...
                    <div className="flex justify-between items-start border-b border-gray-200 pb-8 mb-8">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-sm text-gray-500">
                                ...
                                Kec. Lembang, Kab. Bandung Barat 40931<br />
                    ...
                    <div className="grid grid-cols-2 gap-12 mb-12">
                        <div>
                            <h3 className="text-xs uppercase font-bold text-gray-400 tracking-wider mb-2">Ditagihkan Kepada</h3>
                            <div className="font-semibold text-lg">{order.user.name || 'Admin / Guest'}</div>
                            <div className="text-gray-500">{order.user.phone}</div>
                            <div className="text-gray-500 mt-2 text-sm">{order.user.email}</div>
                        </div>
                        <div className="text-right">
                            <h3 className="text-xs uppercase font-bold text-gray-400 tracking-wider mb-2">Detail</h3>
                            ...
                            <div className="flex justify-between md:justify-end gap-8">
                                ...
                                <span className="font-medium uppercase">{order.paymentStatus === 'PAID' ? 'LUNAS' : order.paymentStatus === 'PENDING' ? 'MENUNGGU' : 'BATAL'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <table className="w-full mb-12">
                        <thead>
                            <tr className="border-b-2 border-gray-900 text-left">
                                ...
                                <th className="py-3 text-right font-bold uppercase text-xs tracking-wider">Harga</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                            {order.items.map((item: any) => (
                                <tr key={item.id}>
                                    <td className="py-4">
                                        <div className="font-bold">{item.goat.breed} ({item.goat.name})</div>
                                        <div className="text-sm text-gray-500">
                                            Kode: {item.goat.registrationCode} <br />
                                            Berat: {item.weightAtPurchase} kg
                                        </div>
                                    </td>
                    ...
                    <div className="mt-16 text-center text-sm text-gray-400">
                        <p>Terima kasih telah mempercayai Zadhifa Farm. 100% Halal & Terpercaya.</p>
                    </div>
                </div>
            </div>
        </div>
